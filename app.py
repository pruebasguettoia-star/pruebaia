...
def api_summary():
    """Resumen en texto plano del estado del portfolio — consumible desde Telegram/scripts."""
    cache_lock.read_acquire()
    try:
        market_data = cache.get("data") or {}
    ...
    try:
        ...
        regime    = indicators.get_spy_regime()
    except Exception as e:
        return jsonify({"error": str(e)}), 500
    ...
    open_value = sum(
        p["shares"] * ticker_map[p["ticker"]]["price"]
    ...
    lines = [
        f"📊 <b>Market Tracker — {last_updated}</b>",
        f"💶 Capital: {capital:.0f}€ · Posiciones: {open_value:.0f}€ · Total: {total_equity:.0f}€ ({'+' if total_ret>=0 else ''}{total_ret}%)",
        f"🌡 VIX: {vix:.1f}" if vix else "🌡 VIX: —",
        f"📈 Régimen: {regime.get('regime','—')} ({regime.get('note','—')[:60]})",
        "",
    ]
    ...
    if positions:
        ...
        for p in positions:
            ...
            lines.append(f"  {p['ticker']} {sign}{ret:.1f}%{trail} · {p.get('hours_held',0):.0f}h")
    else:
        lines.append("Sin posiciones abiertas.")

    total_trades = stats.get("total_trades", {})
    if isinstance(total_trades, dict) and total_trades.get("total_trades"):
        wr = round(total_trades["wins"] / total_trades["total_trades"] * 100, 1) if total_trades.get("total_trades") else 0
        lines += [
            "",
            f"<b>Histórico:</b> {total_trades['total_trades']} trades · WR {wr}% · P&L {total_trades.get('total_pnl_eur',0):.0f}€",
        ]
    ...
    return jsonify({"summary": text, "equity": total_equity, "ret_pct": total_ret,
                    "open_positions": len(positions), "vix": vix,
                    "regime": regime.get("regime"), "last_updated": last_updated})


@app.route("/api/summary")
def api_summary():
    """Resumen del portfolio en texto plano — consumible desde Telegram o scripts."""
    cache_lock.read_acquire()
    try:
        market_data = cache.get("data") or {}
        last_updated = cache.get("last_updated", "—")
    ...
    try:
        ...
        stats       = storage.get_performance_stats()
        vix         = indicators.get_vix_sma5()
        regime_info = indicators.get_spy_regime()
    except Exception as e:
        return jsonify({"error": str(e)}), 500
    ...
    open_value = sum(
        p["shares"] * (ticker_map[p["ticker"]]["price"] if p["ticker"] in ticker_map
                       else p["entry_price"])
        for p in positions
    )
    total_equity = round(capital + open_value, 2)
    total_ret    = round((total_equity - paper_engine.PAPER2_INITIAL_CAP) / paper_engine.PAPER2_INITIAL_CAP * 100, 2)

    tr = stats.get("total_trades") or {}
    lines = [
        f"📊 <b>Resumen — {last_updated}</b>",
        f"Capital: €{capital:,.2f} · Posiciones: €{open_value:,.2f} · Total: €{total_equity:,.2f} ({'+' if total_ret>=0 else ''}{total_ret}%)",
        f"VIX: {vix:.1f}" if vix else "VIX: —",
        f"Régimen: {regime_info.get('regime','—')}",
        "",
        f"<b>Posiciones abiertas ({len(positions)})</b>",
    ]
    for p in sorted(positions, key=lambda x: x.get("ret_pct", 0), reverse=True):
        sign = "+" if p.get("ret_pct", 0) >= 0 else ""
        trail = " 🔒" if p.get("trailing_active") else ""
        lines.append(f"  {p['ticker']}: {sign}{p.get('ret_pct',0):.1f}%{trail} ({p.get('hours_held',0):.0f}h)")

    if tr:
        lines += [
            "",
            f"<b>Histórico</b>: {tr.get('total_trades',0)} trades · WR {round(tr.get('wins',0)/max(tr.get('total_trades',1),1)*100)}% · P&L €{tr.get('total_pnl_eur',0):+.0f}",
        ]
